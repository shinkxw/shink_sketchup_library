module Shink::BaseLibrary::Zip
  class Entry
    ...
    def check_name(name)
      ...
      raise EntryNameError, "Illegal ZipEntry name '#{name}', name must not start with /"
    end
    ...
    def initialize(*args)
      ...
      @fstype = RUNNING_ON_WINDOWS ? FSTYPE_FAT : FSTYPE_UNIX
      ...
      @crc                = args[5] || 0
      @compression_method = args[6] || Entry::DEFLATED
      @size               = args[7] || 0
      @time               = args[8] || DOSTime.now

      @ftype = name_is_directory? ? :directory : :file
      @extra = ExtraField.new(@extra.to_s) unless @extra.is_a?(ExtraField)
    end
    ...
    def read_local_entry(io) #:nodoc:all
      ...
      static_sized_fields_buf = io.read(LOCAL_ENTRY_STATIC_HEADER_LENGTH) || ''

      unless static_sized_fields_buf.bytesize == LOCAL_ENTRY_STATIC_HEADER_LENGTH
      ...
      if extra && extra.bytesize != @extra_length
        raise Error, 'Truncated local zip entry header'
      else
        if @extra.is_a?(ExtraField)
          @extra.merge(extra) if extra
        else
          @extra = ExtraField.new(extra)
        end
      end
    ...
    def check_c_dir_entry_static_header_length(buf)
      return if buf.bytesize == CDIR_ENTRY_STATIC_HEADER_LENGTH
      raise Error, 'Premature end of file. Not enough data for zip cdir entry header'
    end

    def check_c_dir_entry_signature
      return if header_signature == CENTRAL_DIRECTORY_ENTRY_SIGNATURE
      raise Error, "Zip local header magic not found at location '#{local_header_offset}'"
    end

    def check_c_dir_entry_comment_size
      return if @comment && @comment.bytesize == @comment_length
      raise Error, 'Truncated cdir zip entry header'
    end
    ...
    def read_c_dir_extra_field(io)
      if @extra.is_a?(ExtraField)
        ...
        @extra = ExtraField.new(io.read(@extra_length))
      end
    end

    def read_c_dir_entry(io) #:nodoc:all
      static_sized_fields_buf = io.read(CDIR_ENTRY_STATIC_HEADER_LENGTH)
    ...
    def get_input_stream(&block)
      if @ftype == :directory
        yield NullInputStream if block_given?
        NullInputStream
      elsif @filepath
        case @ftype
        when :file
        ...
        zis = InputStream.new(@zipfile, local_header_offset)
    ...
    def write_to_zip_output_stream(zip_output_stream) #:nodoc:all
      if @ftype == :directory
        zip_output_stream.put_next_entry(self, nil, nil, Entry::STORED)
      elsif @filepath
        zip_output_stream.put_next_entry(self, nil, nil, compression_method || Entry::DEFLATED)
        get_input_stream { |is| IOExtras.copy_stream(zip_output_stream, is) }
      else
    ...
    def set_time(binary_dos_date, binary_dos_time)
      @time = DOSTime.parse_binary_dos_format(binary_dos_date, binary_dos_time)
